...
A RISC-V zkVM for client-side verifiable Ethereum.

stark-v generates STARK proofs of RV32IM program execution. Any program that
compiles to RV32IM — including a full Ethereum state transition function (STF) —
can be proved once by a prover node and verified in milliseconds by a light
client, without re-executing the block.

> :warning: This is a work in progress and not yet ready for production.

## Why

A light client today must trust either a majority of validators or a centralized
RPC. With a succinct proof of the STF, it only needs to trust the math: download
the block header, fetch the proof, verify it locally, and accept the new state.
That is the path to a trust-minimized Ethereum on phones, browsers and embedded
devices.

stark-v targets that path end-to-end:

- **RV32IM front-end** — any STF implementation (revm, EELS, a clean-room
  client) can be compiled with the standard Rust RISC-V toolchain and proved
  without writing custom circuits.
- **Small, auditable AIR** — opcode constraints are generated by declarative
  macros, keeping the trusted code base reviewable instead of a sea of
  hand-written constraint polynomials.
- **Fast verifier** — succinct proofs designed to verify on resource-constrained
  clients.

Live benchmarks against other zkVMs on real Ethereum workloads are tracked at
<https://ethproofs.org/csp-benchmarks>.

## Architecture

stark-v is built on the [Stwo](https://github.com/starkware-libs/stwo) Circle
STARK backend, and the AIR component design is inspired by
[OpenVM](https://github.com/openvm-org/openvm). Everything above the proof
system — the RV32IM execution model, the trace layout, the opcode AIRs, the
guest ABI, the host runner — is part of stark-v itself.
...
second). Real-world Ethereum STF numbers are tracked publicly at
<https://ethproofs.org/csp-benchmarks>.
