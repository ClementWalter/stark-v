...
//!
//! This module provides a dependency-free SHA-256 implementation suitable for
//! `no_std` environments.
...
/// Compute SHA-256 digest for the provided bytes.
pub fn sha256(data: &[u8]) -> [u8; 32] {
    ...
    // The length must be encoded in big-endian to match the SHA-256 specification.
...
fn compress_block(state: &mut [u32; 8], block: &[u8]) {
    ...
    for (i, chunk) in block.chunks_exact(4).take(16).enumerate() {
        w[i] = u32::from_be_bytes(chunk.try_into().expect("chunk size"));
    }
    ...
    for i in 16..64 {
        ...
        // The message schedule uses modular addition to follow the SHA-256 round function.
        w[i] = w[i - 16]
            .wrapping_add(s0)
            .wrapping_add(w[i - 7])
            .wrapping_add(s1);
    ...
    for i in 0..64 {
        ...
        c = b;
        b = a;
        // We update `a` last to preserve the round value for the next iteration.
