...
/// Calculates memory expansion gas for a `[start, start + size)` range.
///
/// Why: execution-spec `calculate_gas_extend_memory` explicitly skips entries
/// where `size == 0`; charging expansion for zero-length ranges creates
/// deterministic +3 overcharges when `start` is non-zero and memory is empty.
pub fn memory_expansion_cost_for_range(old_size: usize, start: usize, size: usize) -> u64 {
    if size == 0 {
        return 0;
    }
    memory_expansion_cost(old_size, start.saturating_add(size))
}
...
mod tests {
    ...
    fn test_sstore_gas_costs() {
        ...
        assert_eq!(
            restore_original.refund_delta,
            (GAS_SSTORE_SET - GAS_SLOAD_WARM) as i64
        );
    }
    ...
    #[test]
    fn test_memory_expansion_range_zero_size_costs_zero() {
        assert_eq!(memory_expansion_cost_for_range(0, 1, 0), 0);
        assert_eq!(memory_expansion_cost_for_range(0, 31, 0), 0);
        assert_eq!(memory_expansion_cost_for_range(0, usize::MAX, 0), 0);
    }

    #[test]
    fn test_memory_expansion_range_matches_nonzero_cost() {
        assert_eq!(memory_expansion_cost_for_range(0, 0, 32), 3);
        assert_eq!(memory_expansion_cost_for_range(0, 1, 31), 3);
        assert_eq!(memory_expansion_cost_for_range(0, 1, 32), 6);
    }
