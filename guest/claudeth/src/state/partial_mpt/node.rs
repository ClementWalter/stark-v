...
impl Node {
    ...
    /// Computes the node reference digest from its RLP bytes.
    ///
    /// Why: zero-padding short RLP nodes creates pseudo-hashes that diverge from
    /// execution-spec hashing behavior.
...
mod tests {
    ...
    fn test_compute_hash_inline_node() {
        // Small nodes must still hash their RLP bytes to avoid pseudo-hash behavior.
        let node = Node::new_leaf(vec![1], vec![0x42]);
        let encoded = node.encode_rlp();

        if encoded.len() < 32 {
            let hash = node.compute_hash();
            assert_eq!(hash, crate::crypto::keccak256(&encoded));
        }
    }
