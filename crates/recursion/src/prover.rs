...
use crate::{
    CircleDoubleTable, FriFoldLineTable, Qm31InvTable, Qm31MulTable, circle_double, fri_fold,
...
fn mix_claim(channel: &mut Blake2sChannel, log_sizes: &[u32; 4]) {
    channel.mix_u32s(log_sizes);
}

/// Trace-tree column log sizes in commit order.
fn column_log_sizes(log_sizes: &[u32; 4]) -> Vec<u32> {
    let widths = [
...
pub fn prove_recursion(traces: RecursionTraces, config: PcsConfig) -> RecursionProof {
    ...
    let (mul, inv, fold, double) = components(&mut location_allocator, &log_sizes);

    let stark_proof = prove(&[&mul, &inv, &fold, &double], channel, commitment_scheme)
...
pub fn verify_recursion(proof: RecursionProof, config: PcsConfig) -> Result<(), VerificationError> {
    ...
    mix_claim(channel, &proof.log_sizes);
    commitment_scheme.commit(commitments[1], &column_log_sizes(&proof.log_sizes), channel);

    let mut location_allocator = TraceLocationAllocator::default();
    let (mul, inv, fold, double) = components(&mut location_allocator, &proof.log_sizes);
...
mod tests {
    ...
    fn random_traces(seed: u64, rows: usize) -> RecursionTraces {
    ...
    fn test_recursion_air_prove_verify_roundtrip() {
        let proof = prove_recursion(random_traces(0, 50), PcsConfig::default());
        verify_recursion(proof, PcsConfig::default()).expect("verification failed");
    }

    #[test]
    fn test_recursion_air_rejects_tampered_claim() {
        let mut proof = prove_recursion(random_traces(1, 50), PcsConfig::default());
