    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.shifts_reg.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in shifts_reg, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.shifts_reg, &mut counters);
...
// Generate e2e tests for all opcodes in shifts_reg family
test_e2e!(sll);
test_e2e!(srl);
test_e2e!(sra);
