    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.mul.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in mul, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.mul, &mut counters);
...
// Generate e2e tests for all opcodes in mul family
test_e2e!(mul);
