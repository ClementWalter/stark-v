...
fn test_lt_imm_witness_gen_empty() {
    let table = runner::trace::Lt_immTable::new();
    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.lt_imm.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in lt_imm, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.lt_imm, &mut counters);
...
// Generate e2e tests for all opcodes in lt_imm family
test_e2e!(slti);
test_e2e!(sltiu);
