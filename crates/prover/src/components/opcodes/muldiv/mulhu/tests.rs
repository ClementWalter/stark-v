...
#[test]
fn test_mulhu_e2e() {
    let tracer = run_test_bin("mulhu");

    // Verify trace was captured
    assert!(
        !tracer.mulhu.is_empty(),
        "Expected MULHU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.mulhu, &mut counters);
