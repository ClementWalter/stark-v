...
#[test]
fn test_mulh_empty_table() {
    let table = MulhTable::default();
...
#[test]
fn test_mulh_e2e() {
    let tracer = run_test_bin("mulh");

    // Verify trace was captured
    assert!(
        !tracer.mulh.is_empty(),
        "Expected MULH trace entries, got none. Make sure the binary executes the instruction."
    );
