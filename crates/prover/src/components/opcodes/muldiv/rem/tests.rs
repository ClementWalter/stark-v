...
#[test]
fn test_rem_empty_table() {
    let table = RemTable::default();
...
#[test]
fn test_rem_e2e() {
    let tracer = run_test_bin("rem");

    // Verify trace was captured
    assert!(
        !tracer.rem.is_empty(),
        "Expected REM trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.rem, &mut counters);
