...
#[test]
fn test_divu_empty_table() {
    let table = DivuTable::default();
...
#[test]
fn test_divu_interaction_trace() {
    let table = DivuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_divu_e2e() {
    let tracer = run_test_bin("divu");

    // Verify trace was captured
    assert!(
        !tracer.divu.is_empty(),
        "Expected DIVU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.divu, &mut counters);
