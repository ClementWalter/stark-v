...
#[test]
fn test_mulhsu_empty_table() {
    let table = MulhsuTable::default();
...
#[test]
fn test_mulhsu_interaction_trace() {
    let table = MulhsuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_mulhsu_e2e() {
    let tracer = run_test_bin("mulhsu");

    // Verify trace was captured
    assert!(
        !tracer.mulhsu.is_empty(),
        "Expected MULHSU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.mulhsu, &mut counters);
