...
#[test]
fn test_remu_empty_table() {
    let table = RemuTable::default();
...
#[test]
fn test_remu_interaction_trace() {
    let table = RemuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_remu_e2e() {
    let tracer = run_test_bin("remu");

    // Verify trace was captured
    assert!(
        !tracer.remu.is_empty(),
        "Expected REMU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.remu, &mut counters);
