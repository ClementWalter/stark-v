    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.branch_lt.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in branch_lt, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.branch_lt, &mut counters);
...
// Generate e2e tests for all opcodes in branch_lt family
test_e2e!(blt);
test_e2e!(bge);
test_e2e!(bltu);
test_e2e!(bgeu);
