...
#[test]
fn test_sll_empty_table() {
    let table = SllTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sll_interaction_trace() {
    let table = SllTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sll_e2e() {
    let tracer = run_test_bin("sll");
    assert!(!tracer.sll.is_empty(), "Expected SLL trace entries");

    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sll, &mut counters);
    let log_size = trace
        .first()
