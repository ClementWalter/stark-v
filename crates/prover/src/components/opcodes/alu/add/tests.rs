...
#[test]
fn test_add_empty_table() {
    let table = AddTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_add_interaction_trace() {
    let table = AddTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_add_e2e() {
    let tracer = run_test_bin("add");
    assert!(!tracer.add.is_empty(), "Expected ADD trace entries");

    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.add, &mut counters);
    let log_size = trace
        .first()
