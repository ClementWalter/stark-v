...
#[test]
fn test_srl_empty_table() {
    let table = SrlTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_srl_interaction_trace() {
    let table = SrlTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_srl_e2e() {
    let tracer = run_test_bin("srl");
    assert!(!tracer.srl.is_empty(), "Expected SRL trace entries");

    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.srl, &mut counters);
    let log_size = trace
        .first()
