...
#[test]
fn test_sra_empty_table() {
    let table = SraTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sra_interaction_trace() {
    let table = SraTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sra_e2e() {
    let tracer = run_test_bin("sra");
    assert!(!tracer.sra.is_empty(), "Expected SRA trace entries");

    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sra, &mut counters);
    let log_size = trace
        .first()
