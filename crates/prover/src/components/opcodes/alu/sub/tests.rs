...
#[test]
fn test_sub_empty_table() {
    let table = SubTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sub_interaction_trace() {
    let table = SubTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sub_e2e() {
    let tracer = run_test_bin("sub");
    assert!(!tracer.sub.is_empty(), "Expected SUB trace entries");

    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sub, &mut counters);
    let log_size = trace
        .first()
