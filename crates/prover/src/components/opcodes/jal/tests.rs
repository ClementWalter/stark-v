    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.jal.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in jal, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.jal, &mut counters);
...
// Generate e2e tests for all opcodes in jal family
test_e2e!(jal);
