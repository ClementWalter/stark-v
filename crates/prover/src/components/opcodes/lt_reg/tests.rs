...
#[test]
fn test_lt_reg_witness_gen_empty() {
    let table = runner::trace::Lt_regTable::new();
...
#[test]
fn test_lt_reg_interaction_trace() {
    let table = runner::trace::Lt_regTable::new();
    let mut counters = crate::relations::Counters::new();
    let trace = witness::gen_trace(table, &mut counters);
    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.lt_reg.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in lt_reg, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.lt_reg, &mut counters);
...
// Generate e2e tests for all opcodes in lt_reg family
test_e2e!(slt);
test_e2e!(sltu);
