    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.branch_eq.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in branch_eq, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.branch_eq, &mut counters);
...
// Generate e2e tests for all opcodes in branch_eq family
test_e2e!(beq);
test_e2e!(bne);
