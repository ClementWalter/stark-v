    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.div.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in div, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.div, &mut counters);
...
// Generate e2e tests for all opcodes in div family
test_e2e!(div);
test_e2e!(divu);
test_e2e!(rem);
test_e2e!(remu);
