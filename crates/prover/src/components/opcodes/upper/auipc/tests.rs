...
#[test]
fn test_auipc_empty_table() {
    let table = AuipcTable::default();
...
#[test]
fn test_auipc_interaction_trace() {
    let table = AuipcTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_auipc_e2e() {
    let tracer = run_test_bin("auipc");

    // Verify trace was captured
    assert!(
        !tracer.auipc.is_empty(),
        "Expected AUIPC trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.auipc, &mut counters);
