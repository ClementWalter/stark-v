...
#[test]
fn test_lui_empty_table() {
    let table = LuiTable::default();
...
#[test]
fn test_lui_e2e() {
    let tracer = run_test_bin("lui");

    // Verify trace was captured
    assert!(
        !tracer.lui.is_empty(),
        "Expected LUI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.lui, &mut counters);
