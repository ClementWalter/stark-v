...
#[test]
fn test_ori_empty_table() {
    let table = OriTable::default();
...
#[test]
fn test_ori_e2e() {
    let tracer = run_test_bin("ori");

    // Verify trace was captured
    assert!(
        !tracer.ori.is_empty(),
        "Expected ORI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.ori, &mut counters);
