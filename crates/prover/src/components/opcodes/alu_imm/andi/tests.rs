...
#[test]
fn test_andi_empty_table() {
    let table = AndiTable::default();
...
#[test]
fn test_andi_interaction_trace() {
    let table = AndiTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_andi_e2e() {
    let tracer = run_test_bin("andi");

    // Verify trace was captured
    assert!(
        !tracer.andi.is_empty(),
        "Expected ANDI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.andi, &mut counters);
