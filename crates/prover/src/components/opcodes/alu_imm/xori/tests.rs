...
#[test]
fn test_xori_empty_table() {
    let table = XoriTable::default();
...
#[test]
fn test_xori_interaction_trace() {
    let table = XoriTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_xori_e2e() {
    let tracer = run_test_bin("xori");

    // Verify trace was captured
    assert!(
        !tracer.xori.is_empty(),
        "Expected XORI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.xori, &mut counters);
