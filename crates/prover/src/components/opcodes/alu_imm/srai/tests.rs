...
#[test]
fn test_srai_empty_table() {
    let table = SraiTable::default();
...
#[test]
fn test_srai_interaction_trace() {
    let table = SraiTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_srai_e2e() {
    let tracer = run_test_bin("srai");

    // Verify trace was captured
    assert!(
        !tracer.srai.is_empty(),
        "Expected SRAI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.srai, &mut counters);
