...
#[test]
fn test_addi_empty_table() {
    let table = AddiTable::default();
...
#[test]
fn test_addi_interaction_trace() {
    let table = AddiTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_addi_e2e() {
    let tracer = run_test_bin("addi");

    // Verify trace was captured
    assert!(
        !tracer.addi.is_empty(),
        "Expected ADDI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.addi, &mut counters);
