...
#[test]
fn test_slti_e2e() {
    let tracer = run_test_bin("slti");

    // Verify trace was captured
    assert!(
        !tracer.slti.is_empty(),
        "Expected SLTI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.slti, &mut counters);
