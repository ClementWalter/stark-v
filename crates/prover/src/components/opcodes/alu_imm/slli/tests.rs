...
#[test]
fn test_slli_e2e() {
    let tracer = run_test_bin("slli");

    // Verify trace was captured
    assert!(
        !tracer.slli.is_empty(),
        "Expected SLLI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.slli, &mut counters);
