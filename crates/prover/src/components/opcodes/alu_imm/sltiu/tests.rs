...
#[test]
fn test_sltiu_e2e() {
    let tracer = run_test_bin("sltiu");

    // Verify trace was captured
    assert!(
        !tracer.sltiu.is_empty(),
        "Expected SLTIU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sltiu, &mut counters);
