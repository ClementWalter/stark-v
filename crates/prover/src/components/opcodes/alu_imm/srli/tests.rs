...
#[test]
fn test_srli_e2e() {
    let tracer = run_test_bin("srli");

    // Verify trace was captured
    assert!(
        !tracer.srli.is_empty(),
        "Expected SRLI trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.srli, &mut counters);
