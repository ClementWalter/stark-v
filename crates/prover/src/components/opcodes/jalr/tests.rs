...
#[test]
fn test_jalr_witness_gen_empty() {
    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.jalr.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in jalr, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.jalr, &mut counters);
...
// Generate e2e tests for all opcodes in jalr family
test_e2e!(jalr);
