...
#[test]
fn test_base_alu_imm_witness_gen_empty() {
    let table = runner::trace::Base_alu_immTable::new();
...
#[test]
fn test_base_alu_imm_interaction_trace() {
    let table = runner::trace::Base_alu_immTable::new();
    let mut counters = crate::relations::Counters::new();
    let trace = witness::gen_trace(table, &mut counters);
    ...
    ($opcode:ident) => {
        paste::paste! {
            ...
            fn [<test_ $opcode _e2e>]() {
                ...
                assert!(
                    !tracer.base_alu_imm.is_empty(),
                    concat!("Expected ", stringify!($opcode), " trace entries in base_alu_imm, got none.")
                );

                let mut counters = crate::relations::Counters::new();
                let trace = witness::gen_trace(tracer.base_alu_imm, &mut counters);
...
// Generate e2e tests for all opcodes in base_alu_imm family
test_e2e!(addi);
test_e2e!(xori);
test_e2e!(ori);
test_e2e!(andi);
