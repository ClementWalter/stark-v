...
#[test]
fn test_bge_empty_table() {
    let table = BgeTable::default();
...
#[test]
fn test_bge_e2e() {
    let tracer = run_test_bin("bge");

    // Verify trace was captured
    assert!(
        !tracer.bge.is_empty(),
        "Expected BGE trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.bge, &mut counters);
