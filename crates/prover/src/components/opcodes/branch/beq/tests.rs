...
#[test]
fn test_beq_e2e() {
    let tracer = run_test_bin("beq");

    // Verify trace was captured
    assert!(
        !tracer.beq.is_empty(),
        "Expected BEQ trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.beq, &mut counters);
