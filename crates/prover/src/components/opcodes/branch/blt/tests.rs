...
#[test]
fn test_blt_empty_table() {
    let table = BltTable::default();
...
#[test]
fn test_blt_e2e() {
    let tracer = run_test_bin("blt");

    // Verify trace was captured
    assert!(
        !tracer.blt.is_empty(),
        "Expected BLT trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.blt, &mut counters);
