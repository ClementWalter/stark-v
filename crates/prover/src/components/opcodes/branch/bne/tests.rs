...
#[test]
fn test_bne_empty_table() {
    let table = BneTable::default();
...
#[test]
fn test_bne_e2e() {
    let tracer = run_test_bin("bne");

    // Verify trace was captured
    assert!(
        !tracer.bne.is_empty(),
        "Expected BNE trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.bne, &mut counters);
