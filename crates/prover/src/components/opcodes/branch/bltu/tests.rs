...
#[test]
fn test_bltu_empty_table() {
    let table = BltuTable::default();
...
#[test]
fn test_bltu_interaction_trace() {
    let table = BltuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_bltu_e2e() {
    let tracer = run_test_bin("bltu");

    // Verify trace was captured
    assert!(
        !tracer.bltu.is_empty(),
        "Expected BLTU trace entries, got none. Make sure the binary executes the instruction."
    );

    // Generate traces
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.bltu, &mut counters);
