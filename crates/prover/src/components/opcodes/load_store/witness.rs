...
pub fn gen_interaction_trace(
    ...
    let clock_minus_rs1_clock_prev: Vec<PackedM31> = (0..simd_size)
        ...
        .collect();
    let clock_minus_src_clock_prev: Vec<PackedM31> = (0..simd_size)
        .map(|i| cols.clock[i] - cols.src_clock_prev[i])
        .collect();
    let clock_minus_dst_clock_prev: Vec<PackedM31> = (0..simd_size)
        .map(|i| cols.clock[i] - cols.dst_clock_prev[i])
        .collect();
    ...
    // 9. memory_access: -enabler * (src_as, src_addr_selector, src_clock_prev, src_prev_0..3)
    ...
    // 10. memory_access: +enabler * (src_as, src_addr_selector, clock, src_next_0..3)
    ...
    // 11. range_check_20: -1 * (clock - src_clock_prev)
    let rc_20_src_denom = combine!(relations.range_check_20, [&clock_minus_src_clock_prev]);

    // 12. memory_access: -enabler * (dst_as, dst_addr_selector, dst_clock_prev, dst_prev_0..3)
    ...
    // 13. memory_access: +enabler * (dst_as, dst_addr_selector, clock, dst_next_0..3)
    ...
    // 14. range_check_20: -1 * (clock - dst_clock_prev)
    let rc_20_dst_denom = combine!(relations.range_check_20, [&clock_minus_dst_clock_prev]);
...
pub fn register_multiplicities(
    ...
    counters
        .range_check_20
        .register_many(&neg_enabler, &[&clock_minus_src_clock_prev]);
    counters
        .range_check_20
        .register_many(&neg_enabler, &[&clock_minus_dst_clock_prev]);
}
