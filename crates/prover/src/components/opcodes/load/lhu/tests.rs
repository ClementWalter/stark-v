...
#[test]
fn test_lhu_empty_table() {
    let table = LhuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_lhu_interaction_trace() {
    let table = LhuTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_lhu_e2e() {
    let tracer = run_test_bin("lhu");
    assert!(!tracer.lhu.is_empty());
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.lhu, &mut counters);
    let log_size = trace
        .first()
