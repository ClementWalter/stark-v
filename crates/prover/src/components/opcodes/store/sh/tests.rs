...
#[test]
fn test_sh_empty_table() {
    let table = ShTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sh_interaction_trace() {
    let table = ShTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sh_e2e() {
    let tracer = run_test_bin("sh");
    assert!(!tracer.sh.is_empty());
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sh, &mut counters);
    let log_size = trace
        .first()
