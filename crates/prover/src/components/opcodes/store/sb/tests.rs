...
#[test]
fn test_sb_empty_table() {
    let table = SbTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sb_interaction_trace() {
    let table = SbTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sb_e2e() {
    let tracer = run_test_bin("sb");
    assert!(!tracer.sb.is_empty());
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sb, &mut counters);
    let log_size = trace
        .first()
