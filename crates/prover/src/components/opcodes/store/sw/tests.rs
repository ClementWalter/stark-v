...
#[test]
fn test_sw_empty_table() {
    let table = SwTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
    assert!(trace.is_empty());
}

#[test]
fn test_sw_interaction_trace() {
    let table = SwTable::default();
    let trace = witness::gen_trace(table, &mut Counters::new());
...
#[test]
fn test_sw_e2e() {
    let tracer = run_test_bin("sw");
    assert!(!tracer.sw.is_empty());
    let mut counters = Counters::new();
    let trace = witness::gen_trace(tracer.sw, &mut counters);
    let log_size = trace
        .first()
